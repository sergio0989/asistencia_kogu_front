...
|-------|----------------------------------------------|-------------|------------|--------|-------|
| Bf-04 | Front P1 Promotoría (clientes/pólizas/renov.)| Cerrado | 2026-06-10 | `71201ea` (merge a `main`) | Páginas `comercial/` (clientes, cliente ficha360, polizas, poliza detalle, renovaciones, agentes) + servicios + sidebar con visibilidad por rol + tarjeta de dashboard. Shapes verificados contra el API local (B2-02). Toda interpolación dinámica usa `fmt.esc()`; 422→mensajes por campo (`formErrors`); 403→toast+redirección; scoping del lado del servidor. Verificado contra API local: flujo completo (cliente→póliza→recibos→bandeja→renovación→KPIs), scoping de agente.demo (solo su cartera, 403 en ajeno) y visibilidad de sidebar por rol (abogado/cabina no ven Promotoría), guards de formulario (consentimiento DP-07, cancelar sin motivo) y render sin errores/escapado. Mergeada a `main` con `--no-ff` (commit `71201ea`, rama `feat/bf04-front-promotoria`). |
