...
|-------|------------------------------|-------------|------------|--------|-------|
| Bf-03 | Sanitización XSS (innerHTML) | En revisión | 2026-06-10 | `fix/bf03-xss` | Función única `fmt.esc()` en `format.js` (convención global `window.fmt`); 102 interpolaciones escapadas en 10 archivos; `badgeHtml` escapa internamente; consolidados los escapers locales `escDetalle`/`escHtml` de detalle.js en `fmt.esc`. Casos especiales cubiertos (`renderSelect`, formularios dinámicos de nuevo-caso y detalle: labels del esquema JSON + respuestas guardadas). `innerHTML` estáticos marcados con `// estático`. Verificado con arnés que ejecuta las funciones de render reales con los payloads del brief (img/onerror, script, svg, breakout de textarea) en bandeja/detalle/dashboard/bitácora/usuarios/catálogos/cuestionario: todo escapado, sin doble-escape (`&` se ve normal). Pendiente merge a `main` tras revisión externa. |
